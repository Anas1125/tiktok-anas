...
export default function PostUser({post}: PostUserCompTypes){
    useEffect(() => {
        ...
        setTimeout(() => {
            video.addEventListener('mosueenter', () => {video.play })
            video.addEventListener('mosueleave', () => {video.pause })
        },50)
    ...
    return(
        ...
        {!post.video_url ? (
            <div className="absolute flex items-center justify-center top-0 left-0 aspect-[3/4] w-full object-cover rounded-md bg-black">
                <AiOutlineLoading3Quarters className="animate-spin ml-1" size="80" color="#FFFFFF"></AiOutlineLoading3Quarters>
            </div>
        ) : (
